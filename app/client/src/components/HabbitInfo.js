...
export default class HabbitInfo extends React.Component {
  ...
  componentDidMount() {
    axios
      .get(`http://localhost:3001/habbits/${this.props.match.params.id}`)
      .then(response =>
        this.setState({
          habbit: {
  ...
  handleUpdateHabbit(name) {
    axios
      .put(`http://localhost:3001/habbits/${this.state.habbit.id}`, { name: name })
      .then(response =>
        this.setState({
          habbit: { name: name },
        }),
  ...
  render() {
    ...
    return (
      <Grid>
        ...
        />
        <HabbitInfoContent name={habbit.name} id={habbit.id} executions={executions} />
      </Grid>
    );
  }
}
